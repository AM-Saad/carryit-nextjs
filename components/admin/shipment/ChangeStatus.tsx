...
export default function ChangeStatus() {
    ...
    const [statueColor, setStatueColor] = useState('');
    ...
    const update = async () => {
        ...
        if (status === ShipmentStatus.Canceled && currentItem?.status !== ShipmentStatus.Canceled) {
            ...
            await updater(shipmentRepository.change_status(currentItem?.id!, status, { status: true, reason: reason }), false)
            return
        }
        await updater(shipmentRepository.change_status(currentItem?.id!, status, { status: false, reason: null }), false)
        ...
        >
            ...
            >
                <button
                    ...
                    className="flex  items-center justify-center overflow-hidden rounded-full border border-gray-300 transition-all duration-75 focus:outline-none active:scale-95"
                >
                    <Badge label={SHIPMENT_STATUS_LABELS.get(currentItem?.status!)} className="cursor-pointer-hover" color={getShipmentStatusColor(status)} />
                </button>
            </Popover>
        </motion.div>
    );
